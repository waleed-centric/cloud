  ...
  return (
    <div className={`relative ${className || ''}`} ref={dropdownRef}>
      ...
      >
        ...
        <span>{currentProviderInfo.fullName} Builder</span>
        <svg 
          ...
          <div className="py-2">
            {Object.entries(providers).map(([key, provider]) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 hover:bg-slate-600 transition-colors duration-150 flex items-center space-x-3 ${
                  currentProvider === key ? 'bg-slate-600 text-blue-400' : 'text-slate-200'
                }`}
              >
