...
import {
	DetailedAwsService,
	SubService,
	getServiceById as getAwsServiceById,
	getSubServiceById as getAwsSubServiceById,
} from "../data/aws-services-detailed";
import {
	DetailedAzureService,
	AzureSubService,
	getAzureServiceById,
	getAzureSubServiceById,
} from "../data/azure-services-detailed";
import {
	DetailedGcpService,
	GcpSubService,
	getGcpServiceById,
	getGcpSubServiceById,
} from "../data/gcp-services-detailed";
...
const AwsBuilderContext = createContext<AwsBuilderContextValue | undefined>(
	undefined
);
...
export function AwsBuilderProvider({ children }: { children: ReactNode }) {
	...
	const resolveSubServiceById = (
		...
		switch (currentProvider) {
			case "aws":
				return getAwsSubServiceById(serviceId, subServiceId) as
					| SubServiceType
					| undefined;
			case "azure":
				return getAzureSubServiceById(serviceId, subServiceId) as
					| SubServiceType
					| undefined;
			case "gcp":
				return getGcpSubServiceById(serviceId, subServiceId) as
					| SubServiceType
					| undefined;
			default:
				return undefined;
	...
	const resolvePrimaryNameId = (
		service: DetailedService | undefined
	): string | null => {
		if (!service || !Array.isArray((service as any).commonProperties))
			return null;
	...
	const autoAddDefaultEc2SubServices = (parentNode: PlacedNode) => {
		...
		if (ec2Sub)
			addSubServiceNode(ec2Sub, service, baseX, baseY, ec2Props, parentNode.id);
		if (ebsSub)
			addSubServiceNode(
				ebsSub,
				service,
	...
	const exportToDrawIo = () => {
		...
		const nodes = state.placedNodes
			.map((node, index) => {
				return `<mxCell id="${node.id}" value="${
					node.icon.name
				}" style="shape=image;html=1;verticalAlign=top;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;imageAspect=0;aspect=fixed;image=data:image/svg+xml,${encodeURIComponent(
					node.icon.svg
				)}" vertex="1" parent="1">
        <mxGeometry x="${node.x}" y="${node.y}" width="${
					node.icon.width
				}" height="${node.icon.height}" as="geometry" />
      </mxCell>`;
			})
			.join("\n");
		...
		const connections = state.connections
			.map((conn, index) => {
				return `<mxCell id="${conn.id}" value="${
					conn.label || ""
				}" style="endArrow=classic;html=1;rounded=0;strokeColor=${
					conn.color || "#3B82F6"
				}" edge="1" parent="1" source="${conn.fromNodeId}" target="${
					conn.toNodeId
				}">
...
<mxfile host="app.diagrams.net" modified="2024-01-01T00:00:00.000Z" agent="AWS Builder" version="22.1.11" etag="generated" type="device">
  <diagram name="AWS Architecture" id="aws-arch">
	...
	const registerVirtualAnchors = (
		...
		setState((prev) => ({
			...
			virtualAnchors: [
				...
				...prev.virtualAnchors.filter(
					(a) => !anchors.some((n) => n.id === a.id)
				),
				...anchors,
			],
		}));
	...
	const unregisterVirtualAnchorsByPrefix = (prefix: string) => {
		setState((prev) => ({
			...prev,
			virtualAnchors: prev.virtualAnchors.filter(
	...
	const addSubServiceNode = (
		...
		const parentIdToUse =
			parentNodeIdOverride || state.selectedNodeId || undefined;
		const existingCount = state.placedNodes.filter(
			(n) =>
				n.isSubService &&
		...
		if (parentIdToUse) {
			...
			if (parentNode) {
				...
				const primaryVal = primaryId
					? parentNode.properties?.[primaryId]
					: undefined;
				const friendlyParent =
					typeof primaryVal === "string" && primaryVal
						? primaryVal
						: parentNode.icon?.name || service.name || "Instance";
				parentInstanceLabel = friendlyParent
	...
	const updateNodeProperties = (
		...
		setState((prev) => {
			...
			if (targetNode && !targetNode.isSubService) {
				const svc = resolveServiceById(
					targetNode.serviceId || targetNode.icon.id
				);
				const primaryId = resolvePrimaryNameId(svc);
				const changedPrimary =
					primaryId && typeof (properties as any)[primaryId] === "string";
				if (changedPrimary) {
		...
		switch (currentProvider) {
			case "aws":
				...
				if (node.subServiceId && service) {
					subService = getAwsSubServiceById(service.id, node.subServiceId) as
						| SubServiceType
						| undefined;
				}
				break;
			case "azure":
				...
				if (node.subServiceId && service) {
					subService = getAzureSubServiceById(service.id, node.subServiceId) as
						| SubServiceType
						| undefined;
				}
				break;
			case "gcp":
				...
				if (node.subServiceId && service) {
					subService = getGcpSubServiceById(service.id, node.subServiceId) as
						| SubServiceType
						| undefined;
				}
				break;
